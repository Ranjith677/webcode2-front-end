...
function Viewq() {
    ...
    let loadData = async () => {
        try {
            ...
            let uData = await axios.get(`https://stock-over-flow-clone.onrender.com/ques/${params.id}`, {
                headers: {
    ...
    let viewedCount =async ()=>{
        try {
          let viewedC = await axios.get(`https://stock-over-flow-clone.onrender.com/view-count/${params.id}`,{
            headers:{
              'auth':window.localStorage.getItem('app-token'),
    ...
    let formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                await axios.put(`https://stock-over-flow-clone.onrender.com/cmtpost/${params.id}`, values, {
                    headers: {
                        'auth': window.localStorage.getItem('app-token'),
    ...
    let vote = async () => {
        try {
            let val = await axios.get(`https://stock-over-flow-clone.onrender.com/positive-vote/${params.id}`, {
                headers: {
                    'auth': window.localStorage.getItem('app-token'),
                    'userId': window.localStorage.getItem('userId')
                }
    ...
    let nVote = async () => {
        try {
            let val = await axios.delete(`https://stock-over-flow-clone.onrender.com/negative-vote/${params.id}`, {
                headers: {
                    'auth': window.localStorage.getItem('app-token'),
                    'userId': window.localStorage.getItem('userId')
                }
